...
*/
void Engine::RenderThread(vk::Device device, vk::RenderPass renderPass, vk::SwapchainKHR &swapchain,
                          ...
                          vk::Extent2D &windowExtents, bool &engineEnabled, Engine *context)
{
    ...
    vk::UniqueSemaphore imageAcquiredSemaphore = device.createSemaphoreUnique(imageAcquiredSemaphoreCI);

    vk::RenderPassBeginInfo renderPassBI{
        renderPass, {}, vk::Rect2D{{0, 0}, {windowExtents.width, windowExtents.height}}, clearValues};

    vk::Viewport viewport{0, 0, (float)windowExtents.width, (float)windowExtents.height, 0.0f, 1.0f};

    vk::Rect2D scissor{vk::Offset2D{0, 0}, vk::Extent2D{windowExtents.width, windowExtents.height}};
    ...
    vk::UniqueFence graphicsCommandBufferFence = device.createFenceUnique(vk::FenceCreateInfo{});

    vk::UniqueFence computeCommandBufferFence = device.createFenceUnique(vk::FenceCreateInfo{});

    std::vector<vk::ImageView> attachments(2);
    attachments[1] = depthImageView;

    vk::FramebufferCreateInfo framebufferCI{vk::FramebufferCreateFlags{}, renderPass,           attachments,
                                            windowExtents.width,          windowExtents.height, 1};
    ...
    while (engineEnabled)
    {
        vk::CommandBufferAllocateInfo graphicsCommandBufferAI{graphicsCommandPool, vk::CommandBufferLevel::ePrimary, 1};

        vk::UniqueCommandBuffer graphicsCommandBuffer;
        graphicsCommandBuffer.swap(device.allocateCommandBuffersUnique(graphicsCommandBufferAI)[0]);
        ...
        if (vk::Result::eSuccess == device.acquireNextImageKHR(swapchain, UINT64_MAX, imageAcquiredSemaphore.get(),
                                                               nullptr, &currentFramebuffer))
        {
            ...
            submitInfo = {
                1, &(imageAcquiredSemaphore.get()), pipelineStageFlags, 1, &(graphicsCommandBuffer.get()), 0, nullptr};
            ...
            do
            {
                res = device.waitForFences(graphicsCommandBufferFence.get(), VK_TRUE, kFenceTimeout);
...
void Engine::StartRender()
{
    ...
    m_renderThread =
        std::thread(RenderThread, m_device.get(), m_renderPass.get(), std::ref(m_swapchain),
                    std::ref(m_currentFramebuffer), std::ref(m_graphicsQueue), std::ref(m_computeQueue),
                    std::ref(m_presentQueue), std::ref(m_framebuffers), std::ref(m_swapchainImages),
                    std::ref(m_framebufferCount), std::ref(m_enginePipelines), std::ref(m_depthImageView),
